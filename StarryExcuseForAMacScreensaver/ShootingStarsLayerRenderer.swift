...
final class ShootingStarsLayerRenderer {
    ...
    init(
        ...
        self.directionMode =
            ShootingStarDirectionMode(rawValue: directionModeRaw) ?? .random
        ...
        computeSafeMinY()
        os_log(
            "ShootingStarsLayerRenderer init: avg=%{public}.2fs speed=%{public}.1f len=%{public}.1f thick=%{public}.1f bright=%{public}.2f decay=%{public}.3f mode=%{public}d showBounds=%{public}@",
            log: log,
            type: .info,
            avgSeconds,
            Double(speed),
    ...
    private func computeSafeMinY() {
        ...
        if debugOverlayEnabled {
            os_log(
                "ShootingStarsLayerRenderer safeMinY set to %{public}.1f",
                log: log,
                type: .debug,
                Double(safeMinY)
            )
    ...
    func update(dt: CFTimeInterval) -> ([SpriteInstance], Float) {
        ...
        if logThis {
            os_log(
                "ShootingStars update: active=%{public}d spawnedSprites=%{public}d removed=%{public}d keep=%{public}.3f dbgBounds=%{public}@",
                log: log,
                type: .info,
                active.count,
                sprites.count,
    ...
    private func attemptSpawn() {
        for _ in 0..<spawnAttemptsPerFrame {
            if let star = makeStar() {
                ...
                if debugOverlayEnabled {
                    os_log(
                        "ShootingStars spawn: y=%{public}.1f dir=(%{public}.2f,%{public}.2f) len=%{public}.1f speed=%{public}.1f",
                        log: log,
                        type: .debug,
                        Double(star.head.y),
                        Double(star.dir.dx),
    ...
    private func makeStar() -> ShootingStar? {
        ...
        let lengthJitter =
            baseLength * CGFloat.random(in: 0.85...1.15, using: &rng)
        ...
        for _ in 0..<10 {
            ...
            let extremity1 = CGPoint(
                x: head0.x - dir.dx * lengthJitter,
                y: head0.y - dir.dy * lengthJitter
            )
            let extremity2 = CGPoint(
                x: head0.x + dir.dx * lengthJitter,
                y: head0.y + dir.dy * lengthJitter
            )
    ...
    private func appendStarSprites(
        _ s: ShootingStar,
        ...
        for i in 0..<segments {
            ...
            sprites.append(
                SpriteInstance(
                    centerPx: SIMD2<Float>(Float(px), Float(py)),
