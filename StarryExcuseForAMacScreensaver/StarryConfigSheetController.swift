    ...
    NSTextFieldDelegate
{
    ...
    // New building lights density slider (normalized 0..1 like stars)
    var buildingLightsDensitySlider: NSSlider!
    var buildingLightsDensityPreview: NSTextField!
    ...
    var shootingStarsTrailHalfLifeSlider: NSSlider? {
        shootingStarsTrailDecaySlider
    }
    ...
    var shootingStarsTrailHalfLifePreview: NSTextField? {
        shootingStarsTrailDecayPreview
    }
    ...
    var satellitesTrailHalfLifePreview: NSTextField? {
        satellitesTrailDecayPreview
    }
    ...
    private var lastBuildingLightsSpawnFractionOfMax: Double = 0.0
    ...
    // Reference constants for star & building light rate estimation (match engine logic)
    ...
    private func initializeProgrammaticUIIfNeeded() {
        guard !uiInitialized, window != nil else { return }
        uiInitialized = true
        ...
        if let styleMaskRaw = window?.styleMask.rawValue, let log = log {
            os_log(
                "Config sheet UI initialized early (styleMask raw=0x%{public}llx)",
                log: log,
                type: .info,
                styleMaskRaw
            )
    ...
    private func populateDefaultsAndState() {
        ...
        updateBuildingLightsDensityPreview()

        if let bhSlider = buildingHeightSlider {
            bhSlider.doubleValue = defaultsManager.buildingHeight
        }
        if let bhPrev = buildingHeightPreview,
            let bhSlider = buildingHeightSlider
        {
            bhPrev.stringValue = String(
                format: "%.2f%%",
                bhSlider.doubleValue * 100.0
            )
        }
        ...
        if let bfPrev = buildingFrequencyPreview,
            let bfSlider = buildingFrequencySlider
        {
        ...
        if let bright = brightBrightnessSlider {
            ...
            brightBrightnessPreview?.stringValue = String(
                format: "%.3f",
                bright.doubleValue
            )
        }
        if let dark = darkBrightnessSlider {
            ...
            darkBrightnessPreview?.stringValue = String(
                format: "%.3f",
                dark.doubleValue
            )
        }
        ...
        if let phaseCB = moonPhaseOverrideCheckbox {
            phaseCB.state =
                defaultsManager.moonPhaseOverrideEnabled ? .on : .off
        }
        ...
        shootingStarsAvgSecondsField.doubleValue =
            defaultsManager.shootingStarsAvgSeconds
        ...
        if let len = shootingStarsLengthSlider {
            ...
            shootingStarsLengthPreview?.stringValue = String(
                format: "%.0f",
                len.doubleValue
            )
        }
        if let spd = shootingStarsSpeedSlider {
            ...
            shootingStarsSpeedPreview?.stringValue = String(
                format: "%.0f",
                spd.doubleValue
            )
        }
        if let thick = shootingStarsThicknessSlider {
            ...
            shootingStarsThicknessPreview?.stringValue = String(
                format: "%.2f",
                thick.doubleValue
            )
        }
        if let bright = shootingStarsBrightnessSlider {
            ...
            shootingStarsBrightnessPreview?.stringValue = String(
                format: "%.3f",
                bright.doubleValue
            )
        }
        if let hl = shootingStarsTrailHalfLifeSlider {
            ...
            shootingStarsTrailHalfLifePreview?.stringValue = String(
                format: "%.3f",
                hl.doubleValue
            )
        }
        ...
        shootingStarsDebugSpawnBoundsCheckbox?.state =
            defaultsManager.shootingStarsDebugShowSpawnBounds ? .on : .off

        // Satellites
        satellitesEnabledCheckbox?.state =
            defaultsManager.satellitesEnabled ? .on : .off
        satellitesAvgSecondsField?.doubleValue =
            defaultsManager.satellitesAvgSpawnSeconds

        if let speedSlider = satellitesSpeedSlider {
            speedSlider.doubleValue = defaultsManager.satellitesSpeed
            satellitesSpeedPreview?.stringValue = String(
                format: "%.1f",
        ...
        if let sizeSlider = satellitesSizeSlider {
            ...
            satellitesSizePreview?.stringValue = String(
                format: "%.2f",
        ...
        if let brightnessSlider = satellitesBrightnessSlider {
            ...
            satellitesBrightnessPreview?.stringValue = String(
                format: "%.3f",
                brightnessSlider.doubleValue
            )
        }
        ...
        if let hlSlider = satellitesTrailHalfLifeSlider {
            hlSlider.doubleValue =
                defaultsManager.satellitesTrailHalfLifeSeconds
            satellitesTrailHalfLifePreview?.stringValue = String(
                format: "%.3f",
                hlSlider.doubleValue
            )
        }
        ...
        lastBuildingFrequency =
            buildingFrequencySlider?.doubleValue
        ...
        lastBrightBrightness =
            ...
            ?? defaultsManager.moonBrightBrightness
        lastDarkBrightness =
            darkBrightnessSlider?.doubleValue
            ?? defaultsManager.moonDarkBrightness
        lastMoonPhaseOverrideEnabled =
            moonPhaseOverrideCheckbox?.state == .on
            ? true : defaultsManager.moonPhaseOverrideEnabled
        lastMoonPhaseOverrideValue =
            moonPhaseSlider?.doubleValue
            ?? defaultsManager.moonPhaseOverrideValue
        ...
        lastShootingStarsDirectionMode =
            shootingStarsDirectionPopup?.indexOfSelectedItem
            ?? defaultsManager.shootingStarsDirectionMode
        lastShootingStarsLength =
            shootingStarsLengthSlider?.doubleValue
            ?? defaultsManager.shootingStarsLength
        lastShootingStarsSpeed =
            shootingStarsSpeedSlider?.doubleValue
            ?? defaultsManager.shootingStarsSpeed
        lastShootingStarsThickness =
            shootingStarsThicknessSlider?.doubleValue
            ?? defaultsManager.shootingStarsThickness
        lastShootingStarsBrightness =
            shootingStarsBrightnessSlider?.doubleValue
            ?? defaultsManager.shootingStarsBrightness
        lastShootingStarsTrailHalfLifeSeconds =
            shootingStarsTrailHalfLifeSlider?.doubleValue
            ?? defaultsManager.shootingStarsTrailHalfLifeSeconds
        lastShootingStarsDebugSpawnBounds =
            shootingStarsDebugSpawnBoundsCheckbox?.state == .on
            ? true : defaultsManager.shootingStarsDebugShowSpawnBounds
        lastSatellitesEnabled = satellitesEnabledCheckbox?.state == .on
        lastSatellitesAvgSpawnSeconds =
            satellitesAvgSecondsField?.doubleValue
            ?? defaultsManager.satellitesAvgSpawnSeconds
        lastSatellitesSpeed =
            satellitesSpeedSlider?.doubleValue
            ?? defaultsManager.satellitesSpeed
        lastSatellitesSize =
            satellitesSizeSlider?.doubleValue ?? defaultsManager.satellitesSize
        lastSatellitesBrightness =
            satellitesBrightnessSlider?.doubleValue
            ?? defaultsManager.satellitesBrightness
        lastSatellitesTrailing = satellitesTrailingCheckbox?.state == .on
        lastSatellitesTrailHalfLifeSeconds =
            satellitesTrailHalfLifeSlider?.doubleValue
            ?? defaultsManager.satellitesTrailHalfLifeSeconds
    ...
    private func buildUI() {
        ...
        let sdSlider = NSSlider(
            value: defaultsManager.starSpawnFractionOfMax,
        ...
        sdSlider.leadingAnchor.constraint(equalTo: sdSliderRow.leadingAnchor)
            .isActive = true
        sdSlider.trailingAnchor.constraint(equalTo: sdSliderRow.trailingAnchor)
            .isActive = true

        // Building Lights Density slider
        let bldLabelRow = NSStackView()
        bldLabelRow.orientation = .horizontal
        bldLabelRow.alignment = .firstBaseline
        bldLabelRow.spacing = 4
        bldLabelRow.translatesAutoresizingMaskIntoConstraints = false
        let bldLabel = makeLabel("Building lights density")
        let bldPreview = makeSmallLabel("--")
        self.buildingLightsDensityPreview = bldPreview
        bldLabelRow.addArrangedSubview(bldLabel)
        bldLabelRow.addArrangedSubview(bldPreview)
        let bldSlider = NSSlider(
            value: defaultsManager.buildingLightsSpawnFractionOfMax,
            minValue: StarryDefaultsManager.buildingLightsSpawnFractionOfMaxMin,
            maxValue: StarryDefaultsManager.buildingLightsSpawnFractionOfMaxMax,
            target: self,
            action: #selector(buildingLightsDensityChanged(_:))
        )
        bldSlider.translatesAutoresizingMaskIntoConstraints = false
        bldSlider.setContentHuggingPriority(.defaultLow, for: .horizontal)
        self.buildingLightsDensitySlider = bldSlider
        let bldSliderRow = NSStackView(views: [bldSlider])
        bldSliderRow.orientation = .horizontal
        bldSliderRow.alignment = .centerY
        bldSliderRow.spacing = 4
        bldSliderRow.translatesAutoresizingMaskIntoConstraints = false
        bldSlider.leadingAnchor.constraint(equalTo: bldSliderRow.leadingAnchor)
            .isActive = true
        bldSlider.trailingAnchor.constraint(
            equalTo: bldSliderRow.trailingAnchor
        ).isActive = true

        let sbcRow = makeLabeledFieldRow(
            label: "Seconds between clears:",
            fieldWidth: 70,
            small: true
        ) { tf in
        ...
        let bhSlider = NSSlider(
            value: defaultsManager.buildingHeight,
        ...
        bhSlider.leadingAnchor.constraint(equalTo: bhSliderRow.leadingAnchor)
            .isActive = true
        bhSlider.trailingAnchor.constraint(equalTo: bhSliderRow.trailingAnchor)
            .isActive = true
        ...
        let bfSlider = NSSlider(
            value: defaultsManager.buildingFrequency,
        ...
        bfSlider.leadingAnchor.constraint(equalTo: bfSliderRow.leadingAnchor)
            .isActive = true
        bfSlider.trailingAnchor.constraint(equalTo: bfSliderRow.trailingAnchor)
            .isActive = true
        ...
        spawnBoundsSwitch.action = #selector(
            shootingStarsDebugSpawnBoundsToggled(_:)
        )
        ...
        let moonSlider = NSSlider(
            value: defaultsManager.moonDiameterScreenWidthPercent,
        ...
        moonSlider.leadingAnchor.constraint(
            equalTo: moonSliderRow.leadingAnchor
        ).isActive = true
        moonSlider.trailingAnchor.constraint(
            equalTo: moonSliderRow.trailingAnchor
        ).isActive = true
        ...
        let brightSlider = NSSlider(
            value: defaultsManager.moonBrightBrightness,
        ...
        brightSlider.leadingAnchor.constraint(
            equalTo: brightSliderRow.leadingAnchor
        ).isActive = true
        brightSlider.trailingAnchor.constraint(
            equalTo: brightSliderRow.trailingAnchor
        ).isActive = true
        ...
        let darkSlider = NSSlider(
            value: defaultsManager.moonDarkBrightness,
        ...
        darkSlider.leadingAnchor.constraint(
            equalTo: darkSliderRow.leadingAnchor
        ).isActive = true
        darkSlider.trailingAnchor.constraint(
        ...
        let phaseSlider = NSSlider(
            value: defaultsManager.moonPhaseOverrideValue,
        ...
        phaseSlider.leadingAnchor.constraint(
            equalTo: phaseSliderRow.leadingAnchor
        ).isActive = true
        phaseSlider.trailingAnchor.constraint(
            equalTo: phaseSliderRow.trailingAnchor
        ).isActive = true
        ...
        for row in [
            ...
            row.leadingAnchor.constraint(equalTo: moonStack.leadingAnchor)
                .isActive = true
            row.trailingAnchor.constraint(equalTo: moonStack.trailingAnchor)
                .isActive = true
        }
        ...
        let shootingAvgRow = makeLabeledFieldRow(
            label: "Seconds between stars:",
            fieldWidth: 70,
            small: true
        ) { tf in
        ...
        let lengthSlider = NSSlider(
            value: defaultsManager.shootingStarsLength,
        ...
        lengthSlider.leadingAnchor.constraint(
            equalTo: lengthSliderRow.leadingAnchor
        ).isActive = true
        lengthSlider.trailingAnchor.constraint(
            equalTo: lengthSliderRow.trailingAnchor
        ).isActive = true
        ...
        let speedSlider = NSSlider(
            value: defaultsManager.shootingStarsSpeed,
        ...
        speedSlider.leadingAnchor.constraint(
            equalTo: speedSliderRow.leadingAnchor
        ).isActive = true
        speedSlider.trailingAnchor.constraint(
            equalTo: speedSliderRow.trailingAnchor
        ).isActive = true
        ...
        let thickSlider = NSSlider(
            value: defaultsManager.shootingStarsThickness,
        ...
        thickSlider.leadingAnchor.constraint(
            equalTo: thickSliderRow.leadingAnchor
        ).isActive = true
        thickSlider.trailingAnchor.constraint(
            equalTo: thickSliderRow.trailingAnchor
        ).isActive = true
        ...
        let brightSSSlider = NSSlider(
            value: defaultsManager.shootingStarsBrightness,
        ...
        brightSSSlider.leadingAnchor.constraint(
            equalTo: brightSSSliderRow.leadingAnchor
        ).isActive = true
        brightSSSlider.trailingAnchor.constraint(
            equalTo: brightSSSliderRow.trailingAnchor
        ).isActive = true
        ...
        let hlSlider = NSSlider(
            value: defaultsManager.shootingStarsTrailHalfLifeSeconds,
        ...
        hlSlider.leadingAnchor.constraint(equalTo: hlSliderRow.leadingAnchor)
            .isActive = true
        hlSlider.trailingAnchor.constraint(equalTo: hlSliderRow.trailingAnchor)
            .isActive = true
        ...
        for row in [
            lengthSliderRow, speedSliderRow, thickSliderRow, brightSSSliderRow,
            hlSliderRow,
        ] {
            row.leadingAnchor.constraint(equalTo: shootingStack.leadingAnchor)
                .isActive = true
            row.trailingAnchor.constraint(equalTo: shootingStack.trailingAnchor)
                .isActive = true
        }
        ...
        // Avg seconds row
        let satellitesAvgRow = makeLabeledFieldRow(
            label: "Seconds between sats:",
            fieldWidth: 70,
            small: true
        ) { tf in
        ...
        let satSpeedSlider = NSSlider(
            value: defaultsManager.satellitesSpeed,
        ...
        satSpeedSlider.leadingAnchor.constraint(
            equalTo: satSpeedSliderRow.leadingAnchor
        ).isActive = true
        satSpeedSlider.trailingAnchor.constraint(
            equalTo: satSpeedSliderRow.trailingAnchor
        ).isActive = true
        ...
        let satSizeSlider = NSSlider(
            value: defaultsManager.satellitesSize,
        ...
        satSizeSlider.leadingAnchor.constraint(
            equalTo: satSizeSliderRow.leadingAnchor
        ).isActive = true
        satSizeSlider.trailingAnchor.constraint(
            equalTo: satSizeSliderRow.trailingAnchor
        ).isActive = true
        ...
        let satBrightnessSlider = NSSlider(
            value: defaultsManager.satellitesBrightness,
        ...
        satBrightnessSlider.setContentHuggingPriority(
            .defaultLow,
            for: .horizontal
        )
        ...
        satBrightnessSlider.leadingAnchor.constraint(
            equalTo: satBrightnessSliderRow.leadingAnchor
        ).isActive = true
        satBrightnessSlider.trailingAnchor.constraint(
            equalTo: satBrightnessSliderRow.trailingAnchor
        ).isActive = true
        ...
        let satHLSlider = NSSlider(
            value: defaultsManager.satellitesTrailHalfLifeSeconds,
        ...
        satHLSlider.leadingAnchor.constraint(
            equalTo: satHLSliderRow.leadingAnchor
        ).isActive = true
        satHLSlider.trailingAnchor.constraint(
            equalTo: satHLSliderRow.trailingAnchor
        ).isActive = true
        ...
        for row in [
            satSpeedSliderRow, satSizeSliderRow, satBrightnessSliderRow,
            satHLSliderRow,
        ] {
            row.leadingAnchor.constraint(equalTo: satellitesStack.leadingAnchor)
                .isActive = true
            row.trailingAnchor.constraint(
                equalTo: satellitesStack.trailingAnchor
            ).isActive = true
        }
        ...
        for box in boxesToExpand {
            ...
            box.widthAnchor.constraint(equalTo: sectionsStack.widthAnchor)
                .isActive = true
        }

        NSLayoutConstraint.activate([
            sectionsStack.topAnchor.constraint(
                equalTo: docView.topAnchor,
                constant: 12
            ),
        ...
        NSLayoutConstraint.activate([
            ...
            scroll.leadingAnchor.constraint(
                equalTo: leftContainer.leadingAnchor
            ),
            scroll.trailingAnchor.constraint(
                equalTo: leftContainer.trailingAnchor
            ),
        ])
        ...
        let pauseBtn = NSButton(
            title: "Pause",
            target: self,
            action: #selector(previewTogglePause(_:))
        )
        ...
        NSLayoutConstraint.activate([
            ...
            buttonsRow.bottomAnchor.constraint(
                equalTo: leftContainer.bottomAnchor,
                constant: -8
            ),
        ...
        for row in [sbcRow] {
            row.trailingAnchor.constraint(equalTo: generalStack.trailingAnchor)
                .isActive = true
        }
        if let shootingAvgRow = shootingStarsAvgSecondsField?.superview
            as? NSStackView,
            let shootingParent = shootingAvgRow.superview
        {
            shootingAvgRow.trailingAnchor.constraint(
                equalTo: shootingParent.trailingAnchor
            ).isActive = true
        }
        if let satsAvgRow = satellitesAvgSecondsField?.superview
            as? NSStackView,
            let satsParent = satsAvgRow.superview
        {
            satsAvgRow.trailingAnchor.constraint(
                equalTo: satsParent.trailingAnchor
            ).isActive = true
        }
        ...
        if let log = log {
            os_log(
                "buildUI completed (sections=%{public}d)",
                log: log,
                type: .info,
                sectionsStack.arrangedSubviews.count
            )
        }
    }
    ...
    private func makeLabeledFieldRow(
        label: String,
        fieldWidth: CGFloat,
        small: Bool,
        bindField: (NSTextField) -> Void
    ) -> NSStackView {
        let row = NSStackView()
    ...
    private func applyAccessibility() {
        starDensitySlider.setAccessibilityLabel(
            "Star density (fraction of maximum)"
        )
        buildingLightsDensitySlider.setAccessibilityLabel(
            "Building lights density (fraction of maximum)"
        )
        secsBetweenClears?.setAccessibilityLabel("Seconds between clears")
        buildingHeightSlider?.setAccessibilityLabel(
            "Building height fraction of screen"
        )
        ...
        brightBrightnessSlider?.setAccessibilityLabel(
            "Moon bright side brightness"
        )
        darkBrightnessSlider?.setAccessibilityLabel("Moon dark side brightness")
        moonPhaseOverrideCheckbox?.setAccessibilityLabel(
            "Enable moon phase override"
        )
        ...
        shootingStarsAvgSecondsField.setAccessibilityLabel(
            "Average seconds between shooting stars"
        )
        shootingStarsDirectionPopup?.setAccessibilityLabel(
            "Shooting stars direction mode"
        )
        shootingStarsLengthSlider?.setAccessibilityLabel(
            "Shooting stars length in pixels"
        )
        shootingStarsSpeedSlider?.setAccessibilityLabel(
            "Shooting stars speed in pixels per second"
        )
        shootingStarsThicknessSlider?.setAccessibilityLabel(
            "Shooting stars thickness in pixels"
        )
        shootingStarsBrightnessSlider?.setAccessibilityLabel(
            "Shooting stars brightness"
        )
        shootingStarsTrailHalfLifeSlider?.setAccessibilityLabel(
            "Shooting stars trail half-life seconds"
        )
        satellitesEnabledCheckbox?.setAccessibilityLabel(
            "Enable satellites layer"
        )
        satellitesAvgSecondsField?.setAccessibilityLabel(
            "Average seconds between satellites"
        )
        satellitesSpeedSlider?.setAccessibilityLabel(
            "Satellites speed pixels per second"
        )
        satellitesSizeSlider?.setAccessibilityLabel("Satellites size in pixels")
        satellitesBrightnessSlider?.setAccessibilityLabel(
            "Satellites brightness"
        )
        satellitesTrailingCheckbox?.setAccessibilityLabel(
            "Enable satellites trailing"
        )
        satellitesTrailHalfLifeSlider?.setAccessibilityLabel(
            "Satellites trail half-life seconds"
        )
    ...
    private func applySystemSymbolImages() {
        ...
        let symbolNames: Set<String> = [
            "sparkles", "arrow.clockwise", "clock", "building.2",
            "building.2.fill",
            "moonphase.new.moon", "moonphase.full.moon", "sun.min", "sun.max",
            "moon",
            "circle.lefthalf.filled", "timer", "location.north.line",
            "line.horizontal.3",
            "line.horizontal.3.decrease", "tortoise", "hare", "circle",
            "circle.fill",
            "cloud", "cloud.rain",
        ]
    ...
    private func replaceSymbolImages(
        in view: NSView,
        ...
        for sub in view.subviews {
            if let iv = sub as? NSImageView {
                if let ident = iv.identifier?.rawValue,
                    symbolNames.contains(ident)
                {
                    if let sym = NSImage(
                        systemSymbolName: ident,
                        accessibilityDescription: nil
                    )?.withSymbolConfiguration(config) {
                        iv.image = sym
                        iv.contentTintColor = .labelColor
                    }
                } else if let name = iv.image?.name(),
    ...
    @IBAction func starDensityChanged(_ sender: Any) {
        ...
        if val != lastStarSpawnFractionOfMax {
            logChange(
                changedKey: "starSpawnFractionOfMax",
    ...
    @IBAction func buildingLightsDensityChanged(_ sender: Any) {
        let val = buildingLightsDensitySlider.doubleValue
        if val != lastBuildingLightsSpawnFractionOfMax {
            logChange(
                changedKey: "buildingLightsSpawnFractionOfMax",
                oldValue: format(lastBuildingLightsSpawnFractionOfMax),
                newValue: format(val)
            )
            lastBuildingLightsSpawnFractionOfMax = val
        }
        updateBuildingLightsDensityPreview()
        rebuildPreviewEngineIfNeeded()
        updatePreviewConfig()
    ...
    private func updateStarDensityPreview() {
        ...
        starDensityPreview.stringValue = String(
            format: "%.1f%% (≈ %.0f/sec)",
            fraction * 100.0,
            estimatedStarsPerSec
        )
    }

    private func updateBuildingLightsDensityPreview() {
        let fraction = buildingLightsDensitySlider.doubleValue
        let size =
            moonPreviewView?.bounds.size
            ?? CGSize(width: referenceWidth, height: referenceHeight)
        let w = max(1.0, Double(size.width))
        let h = max(1.0, Double(size.height))
        let scaleFactor = (w * h) / (referenceWidth * referenceHeight)
        let estimatedLightsPerSec =
            fraction * maxBuildingLightsAtReference * scaleFactor
        buildingLightsDensityPreview.stringValue = String(
            format: "%.1f%% (≈ %.0f/sec)",
            fraction * 100.0,
            estimatedLightsPerSec
        )
    }
    ...
    @IBAction func buildingHeightChanged(_ sender: Any) {
        ...
        buildingHeightPreview?.stringValue = String(
            format: "%.2f%%",
            newVal * 100.0
        )
        if oldVal != newVal {
            logChange(
                changedKey: "buildingHeight",
    ...
    @IBAction func moonSliderChanged(_ sender: Any) {
        if sender as AnyObject === moonSizePercentSlider {
            ...
            if val != lastMoonSizePercent {
                logChange(
                    changedKey: "moonDiameterScreenWidthPercent",
        ...
        if let bright = brightBrightnessSlider,
            bright.doubleValue != lastBrightBrightness
        {
            logChange(
                changedKey: "moonBrightBrightness",
        ...
        if let dark = darkBrightnessSlider,
            dark.doubleValue != lastDarkBrightness
        {
            logChange(
                changedKey: "moonDarkBrightness",
    ...
    @IBAction func moonPhaseOverrideToggled(_ sender: Any) {
        ...
        if enabled != lastMoonPhaseOverrideEnabled {
            logChange(
                changedKey: "moonPhaseOverrideEnabled",
    ...
    @IBAction func moonPhaseSliderChanged(_ sender: Any) {
        ...
        if val != lastMoonPhaseOverrideValue {
            logChange(
                changedKey: "moonPhaseOverrideValue",
    ...
    @IBAction func showLightAreaTextureFillMaskToggled(_ sender: Any) {
        ...
        if newVal != lastShowLightAreaTextureFillMask {
            logChange(
                changedKey: "showLightAreaTextureFillMask",
        ...
        rebuildPreviewEngineIfNeeded()
        updatePreviewConfig()
        maybeClearAndRestartPreview(
            reason: "showLightAreaTextureFillMaskToggled"
        )
    }
    ...
    @IBAction func debugOverlayToggled(_ sender: Any) {
        ...
        if newVal != lastDebugOverlayEnabled {
            logChange(
                changedKey: "debugOverlayEnabled",
    ...
    @IBAction func shootingStarsToggled(_ sender: Any) {
        ...
        if enabled != lastShootingStarsEnabled {
            logChange(
                changedKey: "shootingStarsEnabled",
    ...
    @IBAction func shootingStarsDirectionChanged(_ sender: Any) {
        ...
        if mode != lastShootingStarsDirectionMode {
            logChange(
                changedKey: "shootingStarsDirectionMode",
    ...
    @IBAction func shootingStarsSliderChanged(_ sender: Any) {
        if let length = shootingStarsLengthSlider,
            length.doubleValue != lastShootingStarsLength
        {
            logChange(
                changedKey: "shootingStarsLength",
        ...
        if let speed = shootingStarsSpeedSlider,
            speed.doubleValue != lastShootingStarsSpeed
        {
            logChange(
                changedKey: "shootingStarsSpeed",
        ...
        if let thick = shootingStarsThicknessSlider,
            thick.doubleValue != lastShootingStarsThickness
        {
            logChange(
                changedKey: "shootingStarsThickness",
        ...
        if let bright = shootingStarsBrightnessSlider,
            bright.doubleValue != lastShootingStarsBrightness
        {
            logChange(
                changedKey: "shootingStarsBrightness",
                oldValue: format(lastShootingStarsBrightness),
                newValue: format(bright.doubleValue)
            )
            lastShootingStarsBrightness = bright.doubleValue
        }
        if let hlSlider = shootingStarsTrailHalfLifeSlider,
            hlSlider.doubleValue != lastShootingStarsTrailHalfLifeSeconds
        {
            logChange(
                changedKey: "shootingStarsTrailHalfLifeSeconds",
    ...
    @IBAction func shootingStarsAvgSecondsChanged(_ sender: Any) {
        ...
        if val != lastShootingStarsAvgSeconds {
            logChange(
                changedKey: "shootingStarsAvgSeconds",
    ...
    @IBAction func shootingStarsDebugSpawnBoundsToggled(_ sender: Any) {
        ...
        if newVal != lastShootingStarsDebugSpawnBounds {
            logChange(
                changedKey: "shootingStarsDebugSpawnBounds",
    ...
    @IBAction func satellitesSliderChanged(_ sender: Any) {
        if let perMinSlider = satellitesPerMinuteSlider {
            ...
            if avgSeconds != lastSatellitesAvgSpawnSeconds {
                logChange(
                    changedKey: "satellitesAvgSpawnSeconds",
        ...
        if let speedSlider = satellitesSpeedSlider,
            speedSlider.doubleValue != lastSatellitesSpeed
        {
            logChange(
                changedKey: "satellitesSpeed",
        ...
        if let sizeSlider = satellitesSizeSlider,
            sizeSlider.doubleValue != lastSatellitesSize
        {
            logChange(
                changedKey: "satellitesSize",
        ...
        if let brightnessSlider = satellitesBrightnessSlider,
            brightnessSlider.doubleValue != lastSatellitesBrightness
        {
            logChange(
                changedKey: "satellitesBrightness",
        ...
        if let trailHalfLifeSlider = satellitesTrailHalfLifeSlider {
            ...
            if secsHL != lastSatellitesTrailHalfLifeSeconds {
                logChange(
                    changedKey: "satellitesTrailHalfLifeSeconds",
    ...
    private func setupPreviewEngine() {
        ...
        if previewMetalLayer == nil {
            ...
            let scale =
                moonPreviewView.window?.screen?.backingScaleFactor
    ...
    private func advancePreviewFrame() {
        ...
        // Update density previews each frame (in case size changed)
        updateStarDensityPreview()
        updateBuildingLightsDensityPreview()

        let scale =
            window?.backingScaleFactor ?? NSScreen.main?.backingScaleFactor
            ?? 2.0
    ...
    private func currentPreviewRuntimeConfig() -> StarryRuntimeConfig {
        ...
        return StarryRuntimeConfig(
            ...
            shootingStarsDirectionMode: shootingStarsDirectionPopup?
                .indexOfSelectedItem ?? lastShootingStarsDirectionMode,
            ...
            satellitesSpeed: satellitesSpeedSlider?.doubleValue
                ?? lastSatellitesSpeed,
            satellitesSize: satellitesSizeSlider?.doubleValue
                ?? lastSatellitesSize,
            satellitesBrightness: satellitesBrightnessSlider?.doubleValue
                ?? lastSatellitesBrightness,
    ...
    private func updateRendererHalfLives() {
        ...
        let satellitesHL =
            satellitesTrailHalfLifeSlider?.doubleValue
            ?? defaultsManager.satellitesTrailHalfLifeSeconds
        previewRenderer?.setTrailHalfLives(
            satellites: satellitesHL,
            shooting: shootingHL
        )
    }
    ...
    private func updatePreviewLabels() {
        ...
        updateBuildingLightsDensityPreview()
        if let bhSlider = buildingHeightSlider,
            let bhPrev = buildingHeightPreview
        {
            bhPrev.stringValue = String(
                format: "%.2f%%",
                bhSlider.doubleValue * 100.0
        ...
        if let bfSlider = buildingFrequencySlider,
            let bfPrev = buildingFrequencyPreview
        {
            bfPrev.stringValue = String(format: "%.3f", bfSlider.doubleValue)
        }
        if let bright = brightBrightnessSlider,
            let brightPrev = brightBrightnessPreview
        {
        ...
        if let satBright = satellitesBrightnessSlider,
            let lbl = satellitesBrightnessPreview
        {
            lbl.stringValue = String(format: "%.3f", satBright.doubleValue)
        }
        if let hl = satellitesTrailHalfLifeSlider,
            let lbl = satellitesTrailHalfLifePreview
        {
            lbl.stringValue = String(format: "%.3f", hl.doubleValue)
        }
    }
    ...
    @IBAction func saveClose(_ sender: Any) {
        ...
        // Shooting stars
        defaultsManager.shootingStarsEnabled =
            (shootingStarsEnabledCheckbox.state == .on)
        defaultsManager.shootingStarsAvgSeconds =
            shootingStarsAvgSecondsField.doubleValue
        if let popup = shootingStarsDirectionPopup {
            defaultsManager.shootingStarsDirectionMode =
        ...
        if let dbg = shootingStarsDebugSpawnBoundsCheckbox {
            defaultsManager.shootingStarsDebugShowSpawnBounds =
                (dbg.state == .on)
        }
    ...
    @IBAction func cancelClose(_ sender: Any) {
        os_log(
            "cancelClose invoked - dismissing without persisting",
    ...
    private func logChange(
        ...
        guard let log = log else { return }
        os_log(
            "Config change %{public}@ : %{public}@ -> %{public}@ | state: %{public}@",
            log: log,
            type: .info,
            changedKey,
            oldValue,
    ...
    private func stateSummaryString() -> String {
        var parts: [String] = []
        parts.append(
            "starSpawnFractionOfMax=\(format(lastStarSpawnFractionOfMax))"
        )
        parts.append(
            "buildingLightsSpawnFractionOfMax=\(format(lastBuildingLightsSpawnFractionOfMax))"
        )
        parts.append(
            "secsBetweenClears=\(format(secsBetweenClears?.doubleValue ?? lastSecsBetweenClears))"
        )
        parts.append(
            "buildingHeight=\(format(buildingHeightSlider?.doubleValue ?? lastBuildingHeight))"
        )
        parts.append(
            "buildingFrequency=\(format(buildingFrequencySlider?.doubleValue ?? lastBuildingFrequency))"
        )
        parts.append(
            "debugOverlayEnabled=\(debugOverlayEnabledCheckbox?.state == .on ? "true" : "false")"
        )
        parts.append(
        ...
        parts.append(
            "moonBright=\(format(brightBrightnessSlider?.doubleValue ?? lastBrightBrightness))"
        )
        parts.append(
            "moonDark=\(format(darkBrightnessSlider?.doubleValue ?? lastDarkBrightness))"
        )
        parts.append(
            "moonPhaseOverrideEnabled=\(moonPhaseOverrideCheckbox?.state == .on ? "true" : "false")"
        )
        parts.append(
            "moonPhaseOverrideValue=\(format(moonPhaseSlider?.doubleValue ?? lastMoonPhaseOverrideValue))"
        )
        parts.append(
        ...
        parts.append(
            "shootingBrightness=\(format(lastShootingStarsBrightness))"
        )
        parts.append(
            "shootingTrailHL=\(format(lastShootingStarsTrailHalfLifeSeconds))"
        )
        parts.append(
            "shootingDebugBounds=\(lastShootingStarsDebugSpawnBounds ? "true" : "false")"
        )
        ...
        parts.append(
            "satelliteTrailing=\(lastSatellitesTrailing ? "true" : "false")"
        )
        parts.append(
            "satelliteTrailHL=\(format(lastSatellitesTrailHalfLifeSeconds))"
        )
