...
class SkylineCoreRenderer {
    ...
    init(
        ...
        os_log(
            "SkylineCoreRenderer init: starsPerSecond=%.2f buildingLightsPerSecond=%.2f flasherDisabled=%{public}@",
            log: log,
            type: .info,
            self.starsPerSecond,
    ...
    func updateRates(starsPerSecond: Double, buildingLightsPerSecond: Double) {
        ...
        if newStars != self.starsPerSecond {
            os_log(
                "SkylineCoreRenderer rate update: stars %.2f -> %.2f",
                log: log,
                type: .info,
                self.starsPerSecond,
                newStars
        ...
        if newLights != self.buildingLightsPerSecond {
            os_log(
                "SkylineCoreRenderer rate update: building lights %.2f -> %.2f",
                log: log,
                type: .info,
                self.buildingLightsPerSecond,
                newLights
        ...
        if changed && traceEnabled {
            os_log(
                "SkylineCoreRenderer accumulators retained: starAcc=%.3f lightAcc=%.3f",
                log: log,
                type: .info,
                starAccumulator,
                buildingLightAccumulator
            )
        }
    }
    ...
    func setDisableFlasherOnBase(_ disabled: Bool) {
        ...
        os_log(
            "SkylineCoreRenderer: disableFlasherOnBase -> %{public}@",
            log: log,
            type: .info,
            disabled ? "true" : "false"
        )
    ...
    func generateSprites(dtSeconds: Double) -> [SpriteInstance] {
        ...
        if traceEnabled && (frameCounter <= 5 || frameCounter % 60 == 0) {
            os_log(
                "generateSprites(dt=%.4f) starSpawn=%d(acc=%.3f) lightSpawn=%d(acc=%.3f) flasherDisabled=%{public}@ rates(star=%.2f light=%.2f)",
                log: log,
                type: .info,
                dtSeconds,
                starSpawnCount,
                starAccumulator,
                buildingLightSpawnCount,
                buildingLightAccumulator,
                disableFlasherOnBase ? "true" : "false",
                starsPerSecond,
                buildingLightsPerSecond
            )
        }
        ...
        {
            os_log(
                "SkylineCoreRenderer: flasher suppressed this frame",
                log: log,
                type: .info
            )
        }
        if traceEnabled && (frameCounter <= 5 || frameCounter % 60 == 0) {
            os_log(
                "SkylineCoreRenderer frame=%{public}d starsAdded=%{public}d lightsAdded=%{public}d flasherAdded=%{public}d total=%{public}d",
                log: log,
                type: .info,
                frameCounter,
    ...
    private func appendStars(into sprites: inout [SpriteInstance], count: Int) {
        ...
        for _ in 0..<count {
            ...
            let color = premulRGBA(
                r: Float(star.color.red),
                g: Float(star.color.green),
                b: Float(star.color.blue),
                a: 1.0
            )
            sprites.append(
                SpriteInstance(
                    centerPx: SIMD2<Float>(cx, cy),
                    halfSizePx: half,
                    colorPremul: color,
                    shape: .rect
                )
    ...
    private func appendBuildingLights(
        into sprites: inout [SpriteInstance],
        count: Int
    ) {
        guard count > 0 else { return }
        for _ in 0..<count {
            ...
            let half = SIMD2<Float>(repeating: 0.5)
            let color = premulRGBA(
                r: Float(light.color.red),
                g: Float(light.color.green),
                b: Float(light.color.blue),
                a: 1.0
            )
            sprites.append(
                SpriteInstance(
    ...
    private func appendFlasher(into sprites: inout [SpriteInstance]) {
        ...
        let color = premulRGBA(
            r: Float(flasher.color.red),
            g: Float(flasher.color.green),
            b: Float(flasher.color.blue),
            a: 1.0
        )
        sprites.append(
            SpriteInstance(
        ...
        if traceEnabled && (frameCounter <= 5 || frameCounter % 60 == 0) {
            os_log(
                "SkylineCoreRenderer flasher at (%.1f, %.1f) r=%.1f",
                log: log,
                type: .info,
                Double(cx),
                Double(cy),
    ...
    private func premulRGBA(r: Float, g: Float, b: Float, a: Float) -> SIMD4<
        Float
    > {
