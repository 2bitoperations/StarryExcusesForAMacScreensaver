...
### Status (Phase 4 — June 2026)
Windowed app and headless `--dump-png` mode both drive a real `Engine` that ports `StarryEngine.swift` + `Skyline.swift` + `SkylineCoreRenderer.swift` + `Buildings.swift` + `ShootingStarsLayerRenderer.swift` + `SatellitesLayerRenderer.swift` + `Moon.swift` + `MoonTexture.swift`: seeded skyline generation, horizon-weighted stars, building windows with random per-tile light states, rate-clocked sprite emission, periodic clear, **a flashing beacon on the tallest building with `Over`-blended snap-on + exponential decay half-life (default 0.08s, snappy-LED feel)**, **additive shooting-star streaks with decaying trails (`keep = 0.5^(dt/halfLife)`)**, **additive satellite point-lights traversing a flasher-constrained band with their own decay half-life**, and a **procedural moon with real-world Julian-day phase math, traversal-arc motion, 3 terminator modes (hard / smooth / banded), and a 64×64-base-albedo R8Unorm texture upsampled per-canvas-width**. Rendering uses four persistent FBOs (skyline + three ping-pong pairs for flasher/satellites/shooting) orchestrated through an 8-pass per-frame encode (sat decay → shoot decay → flasher decay → skyline sprites → sat sprites → shoot sprites → flasher sprite → composite), with stateless N-layer composite blending in Swift Z-order (skyline → flasher → satellites → shooting), and the moon drawn inside the composite pass *after* the N-layer composite (premultiplied alpha). Disabling any layer via `--*-enabled false` (or `--flasher-period-s 0`) skips GPU resource allocation entirely. CLI surfaces all 39 knobs (13 Phase-2 + 15 Phase-3 + 11 Phase-4) via clap with defaults mirroring `StarryDefaultsManager.swift`. No planets yet.
...
**`starry-core/` — library crate (window-agnostic; all simulation + GPU pipelines + WGSL + headless):**
 ...
 - `starry-rs/starry-core/src/lib.rs` - Crate root: 16 `pub mod` declarations (`config`, `types`, `buildings`, `skyline`, `skyline_renderer`, `shooting_stars`, `satellites`, `moon`, `moon_texture`, `moon_renderer`, `engine`, `gpu`, `sprite`, `decay`, `composite`, `headless`). No re-exports — call sites use explicit paths like `starry_core::config::Config`.
 ...
 - `starry-rs/starry-core/src/engine.rs` - Simulation orchestrator: owns `Skyline` + `SkylineRenderer` + `Option<SatellitesRenderer>` + `Option<ShootingStarsRenderer>` + `Option<Moon>` + seeded `StdRng` + dt clock. **Wall-clock time is an injected parameter**: `frame(wall_now: SystemTime)` ticks against wall-clock with `MAX_DT_SECONDS = 0.25` clamp; `frame_with_dt(dt, wall_now)` trusts the caller (no clamp — used by headless at dt=5.0s for visually-rich single-frame output); both delegate to `frame_impl(dt, wall_now)`, which calls `Moon::position`/`Moon::illuminated_fraction` with `wall_now` and builds `MoonParams::from_frame_state(...)`. Returns `FrameOutput { skyline_sprites, clear_skyline, satellites: Option<LayerFrame>, shooting: Option<LayerFrame>, moon: Option<MoonParams> }` where `LayerFrame { sprites, decay }` bundles the per-layer sprite stream with its decay `keep_factor`.
 - `starry-rs/starry-core/src/gpu.rs` - `GpuPipelines` — **window-agnostic** wgpu pipeline owner. Holds `Device` + `Queue` + `TextureFormat` + skyline `Texture` + `Option<DecayLayer>` for flasher, satellites, and shooting (so `--*-enabled false` and `--flasher-period-s 0` fully skip GPU allocation) + skyline `SpriteRenderer` + `CompositeRenderer` + `Option<MoonRenderer>` (also Option-skipped when `--moon-enabled false`). Constructor `new(device, queue, format, width, height, &Config)`; renders into a caller-supplied `&TextureView` via `render_to_view(target_view, FrameOutput)`. Exposes `device()`/`queue()`/`format()` accessors so `WindowedGpu` doesn't duplicate them. `DecayLayer { tex_a, view_a, tex_b, view_b, active_is_a, sprites, decay, blend_mode }` owns each layer's full ping-pong state plus its sprite blend mode (`Additive` for satellites + shooting; `Over` for flasher — snap-on at ON, exponential fade during OFF). Per-frame 8-pass encode: (1) satellites decay → swap; (2) shooting decay → swap; (3) flasher decay → swap; (4) skyline sprite pass with `LoadOp::Clear(LAYER_WIPE_COLOR)` if `clear_skyline` else `Load`; (5) satellites sprite pass (Additive); (6) shooting sprite pass (Additive); (7) flasher sprite pass (Over) — emits at most one sprite per frame during ON-half; (8) composite pass into the target view `LoadOp::Clear(CLEAR_COLOR)` blending `[skyline, flasher?, satellites?, shooting?]` in Z-order, then drawing the moon disc *inside the same composite pass* (premultiplied alpha; visually on top). Resize rebuilds all textures and resets `active_is_a = true`; `MoonRenderer::resize` is delegated. `DecayLayer::new` is now 8-arg (added `blend_mode: BlendMode` + `label: &str`) and carries `#[allow(clippy::too_many_arguments)]` — internal-only with 3 call sites; deemed simpler than a config-struct refactor per the AGENTS.md "simplicity is highly valued" rule.
...
**Top-level:**
 - `starry-rs/README.md` - Build/run instructions, phased roadmap, CLI reference tables (Phase 2 + Phase 3 + Phase 4), headless usage, layout notes.
...
### Conventions specific to the Rust side
 ...
 - **No re-exports in `starry-core/lib.rs`.** 16 `pub mod` declarations, no `pub use`. Call sites use explicit paths (`use starry_core::config::Config`) — keeps the import surface honest and avoids re-export drift.
 ...
 - **Wall-clock time is injected, not fetched.** Any simulation code that depends on real-world time (Phase 4 onwards: the moon phase, future planet ephemerides) takes a `wall_now: SystemTime` parameter rather than calling `SystemTime::now()` internally. The windowed shell (`app.rs`) passes `SystemTime::now()` from the redraw handler; headless (`headless.rs`) passes a hardcoded `UNIX_EPOCH + Duration::from_secs(HEADLESS_NOW_UNIX_SECS)` (2024-01-01 UTC) for byte-stable determinism. Same pattern as `dt` injection in Phase 3 — and same payoff: deterministic headless tests, no environment dependency.
