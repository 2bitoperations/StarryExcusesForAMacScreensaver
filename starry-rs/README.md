...
│       ├── engine.rs             Simulation orchestrator: Skyline + 3 layer renderers + Option<Moon> + planets HashMap + RNG + dt clock (wall_now: SystemTime injected). Phase 6c: `frame_with_dt(dt, wall_now)` doc now lists 3 callers (headless, Phase 6c det+frozen modes, unit tests) — no API change. Phase 6b: optional `DebugSmoothers` struct (FPS 500ms accum + 0.6/0.4 EMA; CPU per-frame 0.8/0.2 EMA) + `FrameOutput::debug_overlay: Option<DebugOverlayFrame>` (owned strings, ~30 bytes/frame, populated when `--debug-overlay-enabled`).
│       ├── gpu.rs                GpuPipelines — window-agnostic wgpu pipelines: DecayLayer ping-pong (3) + Option<MoonRenderer> + Option<PlanetRenderer> + Option<SpriteRenderer> for planet-moons (capacity 5, BlendMode::Over) + Option<DebugOverlayRenderer> (Phase 6b, drawn AFTER moon inside composite pass so glyph text sits on top) + 8-pass render orchestration; renders into a caller-supplied &TextureView
...
│       └── debug.wgsl            Phase 6b: textured-quad shader with per-instance tint. TriangleStrip 4-vertex; corner derived from `vertex_index` via `(v & 1, (v >> 1) & 1)`. Vertex flips top-down screen-px Y to NDC via `1.0 - (y/H)*2`. Fragment samples R8 atlas, multiplies by tint RGBA. Single pipeline drives BOTH text glyphs (atlas alpha + colored tint) AND BG rects (DEL=127 sentinel = solid block + low-α tint).
│
