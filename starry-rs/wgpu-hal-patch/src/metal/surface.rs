...
impl crate::Surface for super::Surface {
    ...
    unsafe fn acquire_texture(
        ...
        // NOTE: wgpu upstream added a macOS occlusion check here (wgpu#8309)
        // that traverses the CALayer hierarchy to find an NSView delegate and
        // checks window.occlusionState.  In legacyScreenSaver.appex the saver
        // runs in a remote-layer-server process; its NSWindow never receives
        // NSWindowOcclusionStateVisible from the OS, so the check incorrectly
        // returns SurfaceError::Occluded on every frame, preventing all
        // rendering.  The check is omitted in this local patch so that
        // nextDrawable() is always attempted.
